...
def test_clotho_dataset():
    ...
    expected_keys = {"audio", "captions", "keywords", "freesound_id", "freesound_link", "start_sample", "end_sample", "manufacturer", "license"}
    ...
    assert isinstance(freesound_id, int) or freesound_id is None, f"Freesound ID field should be an integer or None, got {type(freesound_id)}."
    freesound_link = sample["freesound_link"]
    assert isinstance(freesound_link, str) or freesound_link is None, f"Freesound link field should be a string or None, got {type(freesound_link)}."
    ...
    assert audio_data.shape[0] == expected_num_channels, f"Audio data should have {expected_num_channels} channels, got {audio_data.shape[0]}."
    assert (start_sample is None and end_sample is None) or (start_sample is not None and end_sample is not None and start_sample < end_sample), f"Start sample should be less than end sample, got {start_sample} and {end_sample}."
    ...
    assert len(keywords) > 0, f"Keywords field should not be empty."
