...
MODEL_DISPLAY_NAMES: dict[str, str] = {
    "simclr": "SimCLR", "dino": "DINO", "mae": "MAE", "lejepa": "LeJEPA",
    "nnclr": "NNCLR", "barlow_twins": "Barlow Twins", "supervised": "Supervised",
}
...
def _load_expected_params() -> dict[str, dict[str, dict]]:
    ...
    for yaml_path in sorted(CONF_DIR.glob("*.yaml")):
        ...
        for ds_name, ds_params in params.items():
            ...
            entry["effective_batch_size"] = entry.get("batch_size", 256) * accum if accum > 1 else entry.get("batch_size", 256)
...
def collect_runs(
    ...
    log.info(f"Found {len(runs)} {backbone} runs in {entity}/{project}")
    ...
    log.info(f"Collected {len(df)} valid runs")
    return df
...
def pivot_table(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    ...
    primary = df[~is_seed].sort_values(METRIC, ascending=False).drop_duplicates(
...
def main():
    ...
    parser.add_argument("--refresh", action="store_true", help="Ignore caches")
